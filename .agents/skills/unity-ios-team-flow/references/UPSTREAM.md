...
上述节点是 Unity iOS TeamFlow 1.0.0 初始设计和工具逻辑的来源基线，1.0.1 仍沿用该基线。它表示“已审阅并适配到这个上游节点”，不表示两个项目文件逐行相同，也不承诺自动兼容后续 iOS TeamFlow 版本。
...
5. Unity TeamFlow 是否升版本由实际兼容性变化决定，不直接镜像上游版本号。
