...
本文件只处理 unity-work-flow 自身的 ZIP 和 GitHub Release，不处理业务游戏的 Xcode、TestFlight 或 App Store 发布。
...
- `unity-work-flow-v版本.zip`：工作流源码、测试和完整 Skill，用于开发本工作流。
- `unity-work-flow-skill-v版本.zip`：目录根为 `unity-work-flow/` 的独立 Skill；在业务 Unity 项目的 `.agents/skills/` 下解压。
